...
MoveMeta push(Move move, ChessBoard board,
  ...
  var mso = MoveStackObject(move, board.tiles[move.from], board.tiles[move.to],
      board.chessPiece, List.from(board.possibleOpenings));
  ...
  if (_canTakeEnPassant(mso.movedPiece)) {
    board.chessPiece = mso.movedPiece;
  } else {
...
MoveStackObject pop(ChessBoard board) {
  ...
  board.chessPiece = mso.chessPiece;
  ...
  if (mso.castled) {
    ...
    if (mso.chess) {
      _addPiece(mso.chessPiece, board);
      _setTile(mso.chessPiece.tile, mso.chessPiece, board);
    }
  }
  board.moveCount--;
  return mso;
...
void _checkEnPassant(ChessBoard board, MoveStackObject mso, MoveMeta meta) {
  ...
  if (takenPiece != null && takenPiece == board.chessPiece) {
